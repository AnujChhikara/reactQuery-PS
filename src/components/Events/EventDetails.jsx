...
export default function EventDetails() {
  ...
  const{mutate, isPending: isPendingDeletion, isError: isErrorDeleting, error: errorDeleting} =    useMutation({
      ...
      if(isPending){
       content = <div id="event-detail-content" className='center'><p>loading event details...</p></div>

      }

      if(isError){
        content = <ErrorBlock title="failed to load event details!" message={error.info?.message || 'event details not found.Please try again later.'}/>
      ...
      if(data){
       ...
       content = (
        ...
        <header>
          ...
          <nav>
            <button onClick={handleStartDelete}>{isPending? "Deleting" : "Delete"}</button>
  ...
  return (
    ...
    {isDeleting && (
      ...
      <h2>Are you sure?</h2>
      <p>Do you really wamt to delete this event? This action cannot be undone.</p>
